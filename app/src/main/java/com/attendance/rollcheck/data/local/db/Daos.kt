...
interface StudentDao {
    ...
    @Query(
        "SELECT EXISTS(SELECT 1 FROM students WHERE classId = :classId AND studentId != :studentId AND lower(trim(name)) = lower(trim(:name)))"
    )
    suspend fun studentNameExistsForClassExcept(classId: String, studentId: String, name: String): Boolean
